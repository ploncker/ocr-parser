...
#sys.path.insert(1, os.path.join(sys.path[0], '../..')) #add parents-parent directory to path

import json
import glob
import numpy as np
from src.ocrp import trp
...
def get_lines_in_boundingBox(document, page, paragraph_box):
    ...
    lines =  document.pages[page].getLinesInBoundingBox(bbox)
         
    # # Print only the lines contained in the Bounding Box
    # for line in lines:
    #     print(line.text)

    block_dict = {'text':[], 'ids':[], 'boundingBox':paragraph_box}
    for line in lines:
        #for word in    e.words:
...
if __name__ == "__main__":
    ...
    rec = [x['bbox'] for x in total_text_with_info if x['page']==3]
    ...
    for box in bbox_groups:
        print(box)
        page = 3
        paras.append(get_lines_in_boundingBox(document, page, box))
